package aidevs.course.s01e02.clients.accesslevel;

import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestClient;

@Service
@Slf4j
public class AccessLevelRestClient {

    private final RestClient restClient;

    public AccessLevelRestClient(RestClient.Builder builder, AccessLevelRestClientConfiguration config) {
        this.restClient = builder.baseUrl(config.baseUrl).build();
    }

    public String post(String apikey, String name, String surname, String birthYear) {
        AccessLevelRequest request = AccessLevelRequest.builder()
                .apikey(apikey)
                .name(name)
                .surname(surname)
                .birthYear(birthYear)
                .build();
        log.info("=== POST [{}] ===", request);
        return restClient.post().body(request).retrieve().body(String.class);
    }
}
